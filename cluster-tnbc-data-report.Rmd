...
# remove .1 from end of hta ids
hta.ids <- substr(hta.ids, 1, nchar(hta.ids) - 2)
...
regions of the genome which is an attribute that differentiates the Human Tissue Array from the `r params$old.microarray.platform.name` platform.
...
map.probe.ids <- function(top.gene.df) {
  # remove '.1' from end of HTA 2.0. probe IDs
  gene.ids <- row.names(top.gene.df)
  gene.ids <- substr(gene.ids, 1, nchar(gene.ids) - 2)
  mart <- useDataset('hsapiens_gene_ensembl', useMart('ensembl'))
...
mapped.probe.ids.top.genes <- lapply(X=clust.top.genes, FUN = map.probe.ids)
