...
find.top.genes <- function(expr.mat) {
  ...
  fit <- eBayes(fit)
  return(topTable(fit, coef=2, number=2500, adjust.method="fdr", p.value=0.05,lfc=0))
}

clust.top.genes <- lapply(X=clust.subsets, FUN = find.top.genes)
```

We can observe the overlap of the top 2500 differentially expressed genes among the different clusters to see how 
