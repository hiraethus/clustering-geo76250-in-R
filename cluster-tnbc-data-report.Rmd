...
top.genes.with.probeids <- merge(probe.id.mapping, top.genes, all.x = T, all.y = T, by.x = 'affy_hta_2_0', by.y = 'probeset_id')
```
...
merge.probes <- function(probe.id.mappings, clust.top.genes) {
  return(merge(probe.id.mapping, top.genes, all.x = T, all.y = T, by.x = 'affy_hta_2_0', by.y = 'row.names'))
}

clust.top.genes.with.mapped.probe.ids <- lapply(FUN = function(idx) {
  mappings <- mapped.probe.ids.top.genes[i] 
  top.g    <- clust.top.genes.mapped.probes[i]
  merge.probes(mappings, top.g)
...
<!-- TODO 
* merge top genes with mapped probe ids
* filter out empty / duplicate
